...
def SIFT(src):
    ...
    # keypoint를 fint_local_maxima를 통해 한번 정제화
    ...
    for i in range(len(keypoints)):
        ...
        ## keypoints[i].angle = ???
        ###################################################################
        keypoints[i].angle = int(np.argmax(orient_hist)*10)
        max_hist = np.max(orient_hist)
        for o in range(len(orient_hist)):
            if orient_hist[o] > max_hist*0.8 and np.argmax(orient_hist) != o:
                key_angle = o*10
                response = keypoints[i].response
                keypoints.append(KeyPoint(x, y, None, key_angle, response, 0, -1))
    ...
    for i in range(len(keypoints)):
        ...
        for row in range(-8, 8):
            for col in range(-8, 8):
                ...
                ###################################################################
                gaussian_weight = np.exp((-1 / 16) * (row_rot ** 2 + col_rot ** 2)) # 위에서 활용한 식을 이용
                weight = magnitude[p_y, p_x] * gaussian_weight # 위에서 활용한 식을 이용

                descriptor_angle = angle[p_y, p_x] - keypoints[i].angle
                descriptor_angle = (descriptor_angle + 360) % 360
                descriptor_angle = int(descriptor_angle // 45)
                descriptors[i, descriptor_angle + ((row+8)//4)*32 + ((col+8)//4)*8] += weight
...
def main():
    ...
    # 결과의 학번 작성하기!
    cv2.imshow('201802112_my_sift', result)
    cv2.waitKey()
